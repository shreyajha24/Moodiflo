  ...
  return (
    <div className="sargam-page">
      ...
      <section className="sargam-layout">
        ...
        <aside className="sargam-results"><div className="section-heading"><div><p className="eyebrow">Selected location</p><h2 className="section-title">{selected?.displayName || 'Nothing selected'}</h2></div></div>{selected ? selected.tracks.length ? <div className="track-list">{selected.tracks.map((song) => <button className="track-row" key={song.id} onClick={() => playTrack(song)}><div className="artwork">{song.coverImageUrl && <img src={song.coverImageUrl} alt="" />}</div><span className="track-row-copy"><span className="track-row-title">{song.title}</span><span className="track-row-meta">{song.artist}</span></span><Play size={14} /></button>)}</div> : <p className="empty-copy">Spotify returned no music for this place.</p> : <p className="empty-copy">Your discoveries will appear here.</p>}</aside>
      </section>
    </div>
  );
};
